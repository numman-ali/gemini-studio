...
function DataPoints() {
   const points = useMemo(() => {
      ...
      for (let i = 0; i < 300; i++) {
         ...
         const r = 2.04; // Scaled up from 1.02
         p.push({
      ...
      <group>
         {points.map((pt, i) => (
            <mesh key={i} position={pt.pos} lookAt={() => new THREE.Vector3(0,0,0)}>
               <boxGeometry args={[0.04, 0.04, 0.04 + pt.val * 0.4]} />
    ...
    <div className="h-screen w-full bg-[#0c4a6e] text-white relative overflow-hidden">
       ...
       <Canvas className="absolute inset-0" camera={{ position: [0, 0, 5] }}>
          ...
          <group>
             ...
             <Sphere args={[2.002, 32, 32]}>
                <meshBasicMaterial color="#38bdf8" wireframe transparent opacity={0.1} />
             </Sphere>
             <DataPoints />
          </group>

          <OrbitControls autoRotate autoRotateSpeed={0.5} enableZoom={true} minDistance={3} maxDistance={10} />
       </Canvas>
    </div>
  );
}
